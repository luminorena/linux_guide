...
![alt text](pics_first_chapter/image-27.png)
![alt text](pics_first_chapter/image-28.png)
...
![alt text](pics_first_chapter/image-37.png)
...
![alt text](pics_first_chapter/image-57.png)
...
![alt text](pics_first_chapter/image-63.png)
...
![alt text](pics_first_chapter/image-134.png)
...
![alt text](pics_first_chapter/image-159.png)

Либо через UI, вызвав нужное приложение Calculator

![alt text](pics_first_chapter/image-160.png)
...
![alt text](pics_first_chapter/image-110.png)
...
![alt text](pics_first_chapter/image-122.png)
...
![alt text](pics_first_chapter/image-132.png)
...
![alt text](pics_first_chapter/image-146.png)
...
![alt text](pics_first_chapter/image-147.png)
...
![alt text](pics_first_chapter/image-158.png)
...
Бит setgid может быть установлен с помощью команды chmod:
...
![alt text](pics_first_chapter/image-161.png)
...
![alt text](pics_first_chapter/image-162.png)
...
![alt text](pics_first_chapter/image-163.png)

![alt text](pics_first_chapter/image-164.png)

### Зомби процессы

Каждая программа, которая выполняется в Linux, - это системный процесс, у которого есть свой идентификатор. Каждый процесс может запускать дочерние процессы с помощью функции fork. 

Зомби — это дочерний процесс в Linux, который завершил своё выполнение, но его запись всё ещё остаётся в таблице процессов операционной системы. Это происходит потому, что родительский процесс ещё не прочитал статус завершения дочернего процесса с помощью системного вызова wait(). Таким образом, процесс-зомби — это «мертвый» процесс, который не использует ресурсы CPU и память, кроме небольшой служебной информации, но занимает место в таблице процессов, блокируя PID.

Процесс-зомби существует до тех пор, пока родительский процесс не вызовет wait(), чтобы получить код завершения дочернего процесса и освободить его запись из таблицы процессов. Если родитель игнорирует или неправильно обрабатывает завершение дочернего процесса, зомби может накапливаться, что может привести к исчерпанию доступных PID и проблемам в системе.

Ключевые моменты:

* Зомби-процесс — это завершённый процесс, оставшийся в таблице процессов.

* Не занимает ресурсы CPU и оперативной памяти, кроме служебной информации.

* Родительский процесс должен вызвать wait() для очистки записи зомби.

* Если родитель не делает этого, зомби накапливаются и могут вызвать проблемы.

* Убить зомби напрямую нельзя (даже kill -9 не помогает), нужно воздействовать на родителя (например, послать ему сигнал SIGCHLD или перезапустить)


Краткая инструкция, как уничтожить зомби:


* Найти зомби можно командой ps aux | grep defunct (процессы отмечаются как defunct).

* Определить PID родительского процесса (PPID) через ps -xal | grep defunct.

* Послать родителю сигнал SIGCHLD или завершить родительский процесс, чтобы система автоматически убрала зомби.
...
![alt text](pics_first_chapter/image-168.png)

Если вы попытаетесь убить такой процесс с помощью сигнала KILL, то ничего не выйдет.

![alt text](pics_first_chapter/image-169.png)
...
![alt text](pics_first_chapter/image-170.png)
...
![alt text](pics_first_chapter/image-165.png)
...
![alt text](pics_first_chapter/image-166.png)

![alt text](pics_first_chapter/image-167.png)
...
дает только мгновенный снимок, то есть возвращаемая ею информация действительна лишь на момент вызова команды. Чтобы увидеть работу компьютера в ди-
намике, воспользуемся командой top:

![alt text](pics_first_chapter/image-171.png)
...
![alt text](pics_first_chapter/image-172.png)
...
![alt text](pics_first_chapter/image-173.png)
...
![alt text](pics_first_chapter/image-174.png)

![alt text](pics_first_chapter/image-175.png)
...
[1]+ Hangup
xlogo
Здесь мы запустили программу xlogo в фоновом режиме и затем с помощью команды kill послали ей сигнал HUP. Программа xlogo завершилась, и командная
...
![alt text](pics_first_chapter/image-176.png)
...
файловые системы). Эту задачу решают четыре команды: <br>

 sudo halt              остановить систему (не всегда выключает питание) <br>
 sudo poweroff          выключить систему и питание <br>
 sudo reboot            перезагрузить систему <br>
 sudo shutdown -h now   выключить систему сейчас <br>
 sudo shutdown -r now   перезагрузить систему сейчас <br>
...
![alt text](pics_first_chapter/image-177.png)